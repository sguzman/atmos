use crate::scenes::{
    ...
    loaders::{ConfigLoad, TomlCache, load_entities_config, load_world_config},
...
pub(crate) fn load_world_and_entities(
    ...
    let world_config: WorldConfig =
        match load_world_config(&active_scene.name, toml_cache, asset_server, toml_assets) {
            ConfigLoad::Pending => {
                return None;
            }
    ...
    let entities_config =
        match load_entities_config(&active_scene.name, toml_cache, asset_server, toml_assets) {
            ConfigLoad::Pending => {
                return None;
            }
            ConfigLoad::Ready(config) => config,
    ...
    if let Ok(mut context) = rapier_context.single_mut() {
        if let Some(physics) = world_config.physics.as_ref() {
            if let Some(iterations) = physics.solver_iterations {
                context.integration_parameters.num_solver_iterations = iterations.max(1) as usize;
            }
        }
    }
