...
use crate::scenes::{
    ...
    config::ActiveScene,
    input::{
        CutHover, apply_camera_input, apply_debug_menu_toggle, apply_dialogue_action,
        ...
        apply_sprint_toggle, apply_zoom_action, update_action_states, update_debug_menu_ui,
        update_grab_hold, update_grab_hover, update_grenade_fuses,
    },
};

use super::cut::{
    CutActivationState, CutPreviewAssets, CutState, cleanup_cut_state, update_cut_hover,
    update_cut_preview,
};
...
impl Plugin for ScenePlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(OnExit(AppState::Main), cleanup_cut_state);
        ...
        app.add_systems(
            Update,
            spawn_overlays_from_config.run_if(in_state(AppState::Main)),
        );
        app.add_systems(
            Update,
            update_action_states.run_if(in_state(AppState::Main)),
        );
        app.add_systems(
            Update,
            apply_debug_menu_toggle.run_if(in_state(AppState::Main)),
        );
        app.add_systems(
            Update,
        ...
        app.add_systems(
            Update,
            apply_dialogue_action.run_if(in_state(AppState::Main)),
        );
        ...
        app.add_systems(Update, update_cut_hover.run_if(in_state(AppState::Main)));
        ...
        app.add_systems(
            Update,
            apply_player_respawn.run_if(in_state(AppState::Main)),
        );
        app.add_systems(Update, apply_shoot_action.run_if(in_state(AppState::Main)));
        app.add_systems(
            Update,
            apply_grenade_action.run_if(in_state(AppState::Main)),
        );
        ...
        app.add_systems(Update, update_cut_preview.run_if(in_state(AppState::Main)));
        app.add_systems(
            Update,
            reload::apply_scene_reload.run_if(in_state(AppState::Main)),
        );
        app.add_systems(
            Update,
            despawn_out_of_bounds.run_if(in_state(AppState::Main)),
        );
        app.add_systems(
            Update,
            overlays::toggle_overlays.run_if(in_state(AppState::Main)),
        );
        app.add_systems(
            Update,
            update_grenade_fuses.run_if(in_state(AppState::Main)),
        );
    }
}
