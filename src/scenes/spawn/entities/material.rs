...
pub(super) fn resolve_material(
    ...
    let base_color_fallback =
        Color::srgb_u8(
            shape_color[0],
            shape_color[1],
            shape_color[2],
        );
    let mut resolved = if let Some(
        material,
    ) = material
    {
        resolve_material_from_config(
            material,
            base_color_fallback,
            asset_server,
        )
...
fn resolve_material_from_config(
        ...
        .and_then(parse_color)
    {
        ...
        material.emissive = base
            .to_linear()
            * intensity;
    } else if let Some(intensity) =
        config.emissive_intensity
    {
