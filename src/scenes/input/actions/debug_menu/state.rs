...
use crate::scenes::input::{
    ActionStates, DebugMenuPage, DebugMenuState, SceneCamera, ScenePauseConfig,
};
use crate::scenes::spawn::SunLight;

use super::types::DebugMenuUiTag;

pub(crate) fn debug_menu_enabled(app_config: &AppConfig) -> bool {
...
fn initialize_debug_menu_settings(
    ...
    if let Ok(config) = rapier_config.single() {
        debug_state.settings.gravity =
            Vec3::new(config.gravity.x, config.gravity.y, config.gravity.z);
...
pub(crate) fn safe_despawn(commands: &mut Commands, entity: Entity) {
    commands
        .entity(entity)
