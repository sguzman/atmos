use bevy::{
    ...
    prelude::{ButtonInput, GlobalTransform, Query, Res, ResMut, With},
};

use crate::scenes::input::{
    ActionStates, SceneActionTriggers, TriggerMode, TriggerSource, VolumeShapeKind,
    VolumeTriggerMode,
};
...
pub fn update_action_states(
    ...
    for trigger in triggers.input.iter() {
        let (pressed, just_pressed, just_released) = match trigger.source {
            TriggerSource::Key(key) => resolve_key_state(&keys, key, trigger.mode),
            TriggerSource::Mouse(button) => resolve_mouse_state(&mouse, button, trigger.mode),
        };
        states.update(&trigger.action, pressed, just_pressed, just_released);
    }
    ...
    for trigger in triggers.volumes.iter_mut() {
        if trigger.once && trigger.fired {
            continue;
        }
        let inside = match trigger.shape.kind {
            VolumeShapeKind::Sphere => {
                (player_pos - trigger.position).length() <= trigger.shape.radius
            }
            VolumeShapeKind::Box => {
                let half = trigger.shape.size * 0.5;
                let delta = player_pos - trigger.position;
...
fn resolve_key_state(
    ...
    match mode {
        TriggerMode::Press => {
            let pressed = keys.just_pressed(key);
...
fn resolve_mouse_state(
    ...
    match mode {
        TriggerMode::Press => {
            let pressed = mouse.just_pressed(button);
            (pressed, pressed, false)
        }
        TriggerMode::Hold => (
